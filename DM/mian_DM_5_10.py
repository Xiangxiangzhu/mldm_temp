...
## 选择特征和重新定义标签为二分类问题
features_temp = df.drop(["患病时间-10-15-", "TBA", "PCT", "PLCR", "AFU"], axis=1)
...
df_test_ = df_test_origin_.drop(["患病时间-10-15-", "LDH1", "MG", "PCT", "PLCR", "AFU"], axis=1)
df_test_ = df_test_.dropna()
#########
feature_concate = pd.concat([features_temp, df_test_], axis=0).reset_index(drop=True)

labels = feature_concate["患病时间-5-10-"].replace({1: 0, 2: 1, 3: 1})
feature_concate = feature_concate.drop(["患病时间-5-10-"], axis=1)
